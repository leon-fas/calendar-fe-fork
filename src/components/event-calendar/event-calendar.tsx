...
export function EventCalendar({
  ...
  const handleEventCreate = (startTime: Date) => {
    // Only allow event creation if not in read-only mode
    if (readOnly) return;
  ...
  const calendarContent = (
    <div
      className={cn(
        "flex flex-col sm:flex-row sm:items-center justify-between gap-2 pb-4 sm:px-4",
        className,
      )}
    >
    ...
    <div className="flex flex-1 flex-col">
      {view === "month" && (
        <MonthView
          ...
          events={events}
          onEventSelect={readOnly ? undefined : handleEventSelect}
